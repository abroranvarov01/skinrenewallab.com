...
export default async function ProductReviewPageClient({ params }: { params: Promise<{ slug: string }> }) {
                    ...
                    <div className="flex">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Star
                          key={star}
                          className={`h-5 w-5 ${
                            star <= Math.round(product.rating) ? "text-primary fill-current" : "text-muted-foreground"
                          }`}
                        />
                      ))}
                    </div>
                ...
                <CardContent className="p-6 space-y-4">
                  ...
                  <Button size="lg" className="w-full text-base" asChild>
                    <a
                      ...
                      onClick={(e) => {
                        if (!product.amazonLink || product.amazonLink === "#") {
                          e.preventDefault()
                          console.log("[v0] Amazon link not available for this product")
                        }
                      }}
                    >
